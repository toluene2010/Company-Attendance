...
try:
    ...
    DB_NAME = os.getenv("DB_NAME", "company_attendance")
                        ...
                        with col1:
                            if u['Active']=='TRUE':
                                ...
                                if st.button("Activate", key=f"act_{u['ID']}"):
                                    ...
                                    st.rerun()
                        with col2:
                            if u['ID'] != st.session_state.get('user_id', 0):
    ...
    with tab3:
        ...
        with col1:
            ...
            with st.form("add_department"):
                ...
                section_options = sections_df[['ID', 'Name']].values.tolist() if not sections_df.empty else []
                section_dict = {name: id for id, name in section_options}
                section_name = st.selectbox("Section", list(section_dict.keys()) if section_dict else [])
                desc = st.text_area("Description")
                if st.form_submit_button("Add Department"):
                    if dept_name and section_name:
                        departments_df = read_table("departments")
        ...
        with col2:
            ...
            if not departments_df.empty and not sections_df.empty:
        ...
        if not att.empty:
            if 'Date' in att.columns:
                ...
                if not filtered.empty:
                    ...
                    with col1: st.metric("Present", present, f"{present/total*100:.1f}%" if total > 0 else "0%")
                    with col2: st.metric("Absent", absent, f"{absent/total*100:.1f}%" if total > 0 else "0%")
                    with col3: st.metric("Late", late, f"{late/total*100:.1f}%" if total > 0 else "0%")
                    with col4: st.metric("Leave", leave, f"{leave/total*100:.1f}%" if total > 0 else "0%")
                    # Download CSV
                ...
                st.error("Attendance data is missing the 'Date' column.")
        else:
...
def supervisor_dashboard():
    ...
    with tab3:
        ...
        if not wdf.empty:
            wdf['Active'] = wdf['Active'].astype(str)
            active = wdf[wdf['Active'].str.lower().isin(['true','1','yes'])]
            if not active.empty:
    ...
    with tab2:
        ...
        if not attendance_df.empty and 'Date' in attendance_df.columns:
            ...
            if not monthly.empty:
                ...
                with col2: st.metric("Present", total_present, f"{total_present/total_records*100:.1f}%" if total_records > 0 else "0%")
                with col3: st.metric("Absent", total_absent, f"{total_absent/total_records*100:.1f}%" if total_records > 0 else "0%")
                with col4: st.metric("Late", total_late, f"{total_late/total_records*100:.1f}%" if total_records > 0 else "0%")
